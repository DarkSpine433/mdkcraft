...
export default async function DashboardPage() {
  ...
  const { user } = await payload.auth({ headers }) as { user: any } // Typing user as any for quick property access to custom fields
  ...
  const projects = await payload.find({
    ...
    limit: 5,
  }) as { docs: Project[]; totalDocs: number }
  ...
  return (
    <div className="space-y-12">
      <header>
        <h1 className="text-4xl font-black tracking-tighter mb-2 uppercase">Witaj, {user.name || user.email}!</h1>
        <p className="text-neutral-500 font-mono text-xs uppercase tracking-widest">Twoje Centrum Dowodzenia MDKcraft</p>
      </header>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <div className="p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-primary/50 transition-all group flex flex-col justify-between">
          <div>
            ...
            <div className="text-sm text-neutral-500 uppercase font-mono mb-1">Aktywna Subskrypcja</div>
          <div className="text-xl font-bold">{(user.activeSubscription as SubscriptionPlan)?.name || 'Brak'}</div>
          </div>
        {user.stripeCustomerID && (
            <form action={async () => {
              ...
              <button className="mt-4 text-[10px] font-mono text-primary flex items-center gap-1 hover:underline uppercase">
        ...
        <div className="p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-primary/50 transition-all group">
          ...
          <div className="text-xl font-bold">{tickets.docs.filter(t => t.status !== 'closed' && t.status !== 'resolved').length} Nowych</div>
        </div>
      </div>
      ...
      <section className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          <Link href="/kontakt" className="text-xs font-mono text-primary hover:underline uppercase">Nowy Projekt +</Link>
        </div>

        <div className="grid gap-4">
          {projects.docs.length > 0 ? (
            projects.docs.map(project => (
              <div key={project.id} className="p-6 rounded-2xl bg-[#0a0a0c] border border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                ...
                {project.activityLog && project.activityLog.length > 0 && (
                  <div className="w-full mt-4 pt-4 border-t border-white/5">
                    <div className="text-[10px] font-mono text-neutral-500 uppercase mb-2">Ostatnia aktywność:</div>
                    <div className="text-sm italic">
                      &quot;{project.activityLog[project.activityLog.length - 1].message}&quot;
                      <span className="text-[10px] text-neutral-600 ml-2">
                        {new Date(project.activityLog[project.activityLog.length - 1].date).toLocaleDateString()}
                      </span>
                    </div>
                  </div>
                )}
      ...
      <section className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          <Link href="/account/tickets/new" className="px-4 py-2 bg-white/5 border border-white/10 rounded-lg text-xs font-mono hover:bg-white/10 transition-colors uppercase">Otwórz Ticket +</Link>
        </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-white/5">
                {tickets.docs.map(ticket => (
                  <tr key={ticket.id} className="hover:bg-white/2 transition-colors cursor-pointer group">
                    <td className="px-6 py-4 font-medium group-hover:text-primary transition-colors">{ticket.subject}</td>
                    <td className="px-6 py-4">
                      <span className={`text-[10px] px-2 py-1 rounded-full border ${ticket.status === 'open' ? 'border-green-500/50 text-green-500' : 'border-neutral-500/50 text-neutral-500'}`}>
                        {ticket.status}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-xs">{ticket.priority}</td>
                    <td className="px-6 py-4 text-xs text-neutral-500 font-mono">{new Date(ticket.createdAt).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          <span className="text-xs font-mono text-neutral-500 uppercase">Szyfrowane Przechowywanie</span>
        </div>
            ...
            files.docs.map(file => (
              ...
              >
                ...
                <div className="overflow-hidden">
                  ...
                  <div className="text-[10px] text-neutral-600 font-mono uppercase truncate">{file.description || 'Dokument Projektowy'}</div>
                </div>
              </a>
            ))
          ) : (
